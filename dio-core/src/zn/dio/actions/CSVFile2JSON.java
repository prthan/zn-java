...
public class CSVFile2JSON extends BaseAction
{
  private static final Logger LOG=Logger.get(CSVFile2JSON.class);
  ...
  public void convert(Map<String,String> options) throws IOException, ParseException
  {
    ...
    if(options.get("infile")!=null) csvfilename=options.get("infile");
    if(options.get("headers")!=null) withHeaders=Boolean.parse(options.get("headers"));
